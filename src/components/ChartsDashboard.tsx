...
export default function ChartsDashboard({ tasks }: Props) {
  ...
  {
    label: '<200',
    count: tasks.filter(t => typeof t.roi === 'number' && !isNaN(t.roi) && t.roi < 200).length,
  },
  {
    label: '200-500',
    count: tasks.filter(t => typeof t.roi === 'number' && !isNaN(t.roi) && t.roi >= 200 && t.roi <= 500).length,
  },
  {
    label: '>500',
    count: tasks.filter(t => typeof t.roi === 'number' && !isNaN(t.roi) && t.roi > 500).length,
  },
  {
    label: 'N/A',
    count: tasks.filter(t => t.roi == null || isNaN(t.roi as number)).length,
  },
];
